...
def generate_idea(state: AgentBuilderGraphState) -> AgentBuilderGraphState:
    ...
    try:
        ...
        # Ensure tech_stack is a dict if it's a list (backward compatibility)
        if "tech_stack" in idea_dict and isinstance(idea_dict["tech_stack"], list):
            # Convert list to dict format
            tech_list = idea_dict["tech_stack"]
            idea_dict["tech_stack"] = {
                "frontend": tech_list[0] if len(tech_list) > 0 else "Not specified",
                "backend": tech_list[1] if len(tech_list) > 1 else "Not specified",
                "database": tech_list[2] if len(tech_list) > 2 else "Not specified",
            }
        
        # Ensure required fields have defaults
        if "core_features" not in idea_dict:
            idea_dict["core_features"] = []
        if "architecture_overview" not in idea_dict:
            idea_dict["architecture_overview"] = "Full-stack application architecture"
...
def implement_agent(state: AgentBuilderGraphState) -> AgentBuilderGraphState:
    ...
    tech_stack_str = json.dumps(idea.tech_stack, indent=2) if isinstance(idea.tech_stack, dict) else str(idea.tech_stack)
    core_features_str = ", ".join(idea.core_features) if idea.core_features else "Not specified"
    ...
    try:
        ...
        # Handle backward compatibility - if old format is used, convert it
        if "agent_code" in impl_dict:
            # Old format - convert to new format
            impl_dict["backend_files"] = {
                "agent.py": impl_dict.pop("agent_code", ""),
            }
            if impl_dict.get("schemas_code"):
                impl_dict["backend_files"]["schemas.py"] = impl_dict.pop("schemas_code")
            if impl_dict.get("prompts_code"):
                impl_dict["backend_files"]["prompts.py"] = impl_dict.pop("prompts_code")
            if impl_dict.get("graph_code"):
                impl_dict["backend_files"]["graph.py"] = impl_dict.pop("graph_code")
            if impl_dict.get("tools_code"):
                impl_dict["backend_files"]["tools.py"] = impl_dict.pop("tools_code")
        
        # Ensure all file dicts exist
        if "frontend_files" not in impl_dict:
            impl_dict["frontend_files"] = {}
        if "backend_files" not in impl_dict:
            impl_dict["backend_files"] = {}
        if "database_files" not in impl_dict:
            impl_dict["database_files"] = {}
        if "config_files" not in impl_dict:
            impl_dict["config_files"] = {}
...
def write_files(state: AgentBuilderGraphState) -> AgentBuilderGraphState:
    ...
    # Get repo root (parent of agent-builder)
...
def commit_and_push(state: AgentBuilderGraphState) -> AgentBuilderGraphState:
    ...
    idea = AgentIdea(**state["idea"])
    # Use repo root (parent of agent-builder) for git operations
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
